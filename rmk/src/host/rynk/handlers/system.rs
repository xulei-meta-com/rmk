...
impl<'a> RynkService<'a> {
    pub(crate) async fn handle_get_version(&self, msg: &mut RynkMessage<'_>) -> Result<(), RynkError> {
        msg.write_response(&ProtocolVersion::CURRENT)
    }

    pub(crate) async fn handle_get_capabilities(&self, msg: &mut RynkMessage<'_>) -> Result<(), RynkError> {
        ...
        msg.write_response(&caps)
    }

    pub(crate) async fn handle_reboot(&self, msg: &mut RynkMessage<'_>) -> Result<(), RynkError> {
        // Fire-and-forget: synchronous reset never returns on real hardware.
        crate::boot::reboot_keyboard();
        msg.write_response(&())
    }

    pub(crate) async fn handle_bootloader_jump(&self, msg: &mut RynkMessage<'_>) -> Result<(), RynkError> {
        // Fire-and-forget, same reasoning as `handle_reboot`.
        crate::boot::jump_to_bootloader();
        msg.write_response(&())
    }

    pub(crate) async fn handle_storage_reset(&self, msg: &mut RynkMessage<'_>) -> Result<(), RynkError> {
