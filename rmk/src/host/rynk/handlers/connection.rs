...
impl<'a> RynkService<'a> {
    pub(crate) async fn handle_get_connection_type(&self, msg: &mut RynkMessage<'_>) -> Result<(), RynkError> {
        let t = self.ctx.preferred_connection();
        msg.write_response(&t)
    }

    /// `Cmd::GetConnectionStatus` — the same payload the `ConnectionChange`
    /// topic pushes, so a host can recover a missed push.
    pub(crate) async fn handle_get_connection_status(&self, msg: &mut RynkMessage<'_>) -> Result<(), RynkError> {
        msg.write_response(&self.ctx.connection_status())
    }

    #[cfg(feature = "_ble")]
    pub(crate) async fn handle_get_ble_status(&self, msg: &mut RynkMessage<'_>) -> Result<(), RynkError> {
    ...
    /// dispatch loop.
    #[cfg(feature = "_ble")]
    pub(crate) async fn handle_switch_ble_profile(&self, msg: &mut RynkMessage<'_>) -> Result<(), RynkError> {
    ...
    #[cfg(feature = "_ble")]
    pub(crate) async fn handle_clear_ble_profile(&self, msg: &mut RynkMessage<'_>) -> Result<(), RynkError> {
        let slot = msg.request::<u8>()?;
        if (slot as usize) >= crate::NUM_BLE_PROFILE {
            return Err(RynkError::Invalid);
        }
